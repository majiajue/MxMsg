...
public class NettyClientHandler extends SimpleChannelUpstreamHandler {
		...
		if (xmlMap.get("msgid") != null & !xmlMap.get("msgid").equals("")) {
			...
			if (xmlMap.get("msgid").equals(AncodeFactory.getInstance().LastMsgId)) {
				...
				case DBDataSQL.MSGID_STATUS_GOT:
					...
					// PluginMethod pm = new
					// PluginMethod(AncodeFactory.getInstance().getApplicationContext(),
					// pluginValues);
					// pm.doMethod(xmlMap);
					// }else{
				...
				case DBDataSQL.MSGID_STATUS_SAVED:
					...
					// MixunProtocol.getInstance().delMessage(xmlMap.get("msgid"),
					// new ICallApi() {
					//
					// @Override
					// public void onSuccess(HashMap<String, String> result) {
					// // TODO Auto-generated method stub
					// AncodeFactory.getInstance().LastMsgIdStatus =
					// DataDefine.MSGID_STATUS_ACKED;
					// }
					//
					// @Override
					// public void onFailure() {
	...
	private void ProcessingMsg(final HashMap<String, String> xmlMap) {
		...
		// MixunProtocol.getInstance().delMessage(xmlMap.get("msgid"), new
		// ICallApi() {
		//
		// @Override
		// public void onSuccess(HashMap<String, String> result) {
	...
	private HashMap<String, MessagerHandler> initMessagerHandlerMap(final HashMap<String, String> xmlMap) {
		...
		// messagerHandlerMap.put("maskroom_role", new
		// MaskRoomChangeRole(xmlMap));
		...
		// messagerHandlerMap.put("maskroom_logout_one", new
		// MaskRoomLogout(xmlMap));
